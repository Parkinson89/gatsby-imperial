...
function Slider({ modalExist, firstTitle, pics, className, id, mobileTranslate, mobileCountCenter }) {
  ...
  useEffect(() => {
    ...
    const buttonBack = document.getElementsByClassName(`slider__button--back slider__button--${className}`);
    const buttonNext = document.getElementsByClassName(`slider__button--next slider__button--${className}`);
  ...
  useEffect(() => {
    ...
    if (translateX >= elementsBeforeCenter * defaultTranslate) {
      const button = document.getElementsByClassName(`slider__button--back slider__button--${className}`);
      button[0].disabled = true;
    } else if (translateX <= -(elementsAfterCenter * defaultTranslate)) {
      const button = document.getElementsByClassName(`slider__button--next slider__button--${className}`);
      button[0].disabled = true;
    } else {
      const buttonNext = document.getElementsByClassName(`slider__button--next slider__button--${className}`);
  ...
  }, [translateX, elementsAfterCenter, elementsBeforeCenter, width, className, mobileTranslate]);

  return (
    <div className="slider" id={id}>
      <h1 className="title--blue">{firstTitle}</h1>
      <div className="slider__main-wrapper">
        <button className={`slider__button slider__button--back slider__button--${className}`} type="button">
          <svg
          ...
          <svg
            ...
            xmlns="http://www.w3.org/2000/svg"
          >
            <path className="slider__arrow-path" d="M0 9L15 0.339745L15 17.6603L0 9Z" fill="#AFC5D9" />
          </svg>
          <span className="visually-hidden">назад</span>
        </button>
        <div className={`slider__list-wrapper slider__list-wrapper--background-${className}`}>
          <ul className={`slider__list ${className}__list`} style={{ transform: `translateX(${translateX}px)` }}>
        ...
        <button className={`slider__button slider__button--next slider__button--${className}`} type="button">
          <svg
